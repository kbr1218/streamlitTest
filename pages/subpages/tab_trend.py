...
def show_tab_trend(): 
  # element로 감싸기
  with elements("dashboard_container"):

    # 대시보드 레이아웃 설정
    layout = [
      # 첫 번째 위젯: 월별-타입별 방문객 순위
      # x=0, y=0, 6/12 columns, hight 5
      dashboard.Item("trend_chart", 0, 0, 6, 5),
      # x=6, y=0, 6/12 columns, hight 5
      dashboard.Item("local_map", 6, 0, 6, 5)
    ]

    with dashboard.Grid(layout, draggableHandle=".draggable"):
      #####################################
      #### 01. 월별 타입별 방문객 순위 ####
      with mui.Card(key='chart', sx={"display":"flex", "flexDirection":"colum"}):
        mui.CardHeader(title="방문객 순위", className="draggable")
        with mui.CardContent(sx={"flex":1, "minHeight":1}):


          
          # dashboard 그리드 설정
          # grid = dashboard.Grid(layout, draggable=True, resizable=True)
          # with grid.Item("trend_chart"):
